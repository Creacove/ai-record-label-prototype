                        ...
                        <div className="flex min-w-0 flex-col items-start justify-start gap-2 lg:items-end">
                          ...
                          {(!done && !completionBlocked && task.approvalState !== "blocked" && !availability.disabled) && (
                            <div className="w-full flex flex-col items-end gap-2">
                              <textarea
                                placeholder="Add context or results..."
                                value={taskNotes[task.id] || ""}
                                onChange={(e) => setTaskNotes(prev => ({ ...prev, [task.id]: e.target.value }))}
                                className="w-full min-h-[60px] rounded-[10px] border border-foreground/10 bg-background px-3 py-2 text-[12px] text-foreground placeholder:text-muted-foreground/50 focus:border-brand-accent focus:outline-none focus:ring-1 focus:ring-brand-accent"
                              />
                              <button
                                aria-label={\`Mark done \${task.title}\`}
                                onClick={() => onCompleteTask(task.id, taskNotes[task.id])}
                          ...
                          {(done || completionBlocked || task.approvalState === "blocked" || availability.disabled) && (
                            <button
                              disabled
                              className="rounded-[10px] bg-foreground px-3 py-2 text-[11px] font-bold uppercase tracking-[0.08em] text-background opacity-35 w-full"
                            >
                              Mark done
                            </button>
                          )}
